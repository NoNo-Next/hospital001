...
<script type="text/javascript">
  $(function(){
	  $('#table').bootstrapTable({
		  url: '${pageContext.request.contextPath}/position/queryPositionDept',//访问后台（json）的地址
	 ...
	 $("#btnSaveAthority").click(function(){
		 ...
		 $.ajax({
			  type:"post",//提交方式为post
			  url:"${pageContext.request.contextPath}/position/saveAuthority?menuIds="+menuIds,//访问后台的地址
			  ...
			  var zTreeObj;
			   ...
			   var setting = {
					   check: {
						...
						async: {
							enable: true,
							url: "${pageContext.request.contextPath}/position/sendAuthority?postNumber="+poId  //访问json数据
						}
			   };
...
function myDeleteClick(id) {
	...
	if(confirm("确定要删除么？")){
        $.ajax({
            type:"post",//请求方式
            url:"${pageContext.request.contextPath}/position/deleteById",//根据编号查询用户的信息
...
function addPosition() {
	...
	if(postname==null||postsalary==null||postname==''||postsalary==''){
		...
		//ajax提交数据
		 $.ajax({
		 type:"post",//请求方式
		 url:"${pageContext.request.contextPath}/position/addPosition",//根据编号查询用户的信息
		 data:{
			 postname:postname,
...
function myUpdateClick(id) {
	...
	$.ajax({
		 type:"post",//请求方式
		 url:"${pageContext.request.contextPath}/position/getPositionById",//根据编号查询用户的信息
		 data:{poId:id},//传参数到后台
		 dataType:"json",//以json格式传递数据
		 success:function(data){
			 //data：指后台查询后返回来的用户信息
...
function updatePosition() {
	...
	$.ajax({
		 type:"post",//请求方式
		 url:"${pageContext.request.contextPath}/position/updatePositionById",//根据编号查询用户的信息
		 data:{
			 postnumber:poId,
			 postname:postname,
			 postsalary:salary,
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">职位信息修改</h4>
            </div>
