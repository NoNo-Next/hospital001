...
$(function(){//jquery的加载事件
	$('#table').bootstrapTable({
		  url: '${pageContext.request.contextPath}/user/getUser',//访问后台（json）的地址
		  ...
		  },
		  {
		    ...
		    formatter:function(value,row,index){
		    	...
		    	var imageUrl="<img class='img-rounded' onclick=\"myPhotoClick('/attached/"+row.userphoto+"')\" alt=\"\" src=\"/attached/"+row.userphoto+"\" width=\"50\" height=\"50\">";
		    	return imageUrl;
		    }
		  }, {
			    field: 'usersex',
		...
		$.ajax({
			type:'post',//提交方式
			url:'${pageContext.request.contextPath}/position/queryPosition',//访问后台的地址   查询所有的岗位信息
		...
		$.ajax({
			type:'post',//提交方式
			url:'${pageContext.request.contextPath}/department/queryDepartment',//访问后台的地址   查询所有的岗位信息
			async:false,//  true：异步     false:同步
			dataType: "json",//数据类型
			success: function(data){
				...
				if(data != null){
					...
					$("#myUpDepartment").html(optionhtml);
...
function myDeleteClick(id) {
	if(confirm("确定要删除么？")){
		...
		$.ajax({
		 type:"post",//请求方式
		 // async:false,    // 这个需要写上
		 url:"${pageContext.request.contextPath}/user/deleteById",//根据编号查询用户的信息
...
function myUpdateClick(userId){
	...
	$.ajax({
		 type:"post",//请求方式
		 url:"${pageContext.request.contextPath}/user/getById",//根据编号查询用户的信息
		 ...
		 success:function(data){
			 //data：指后台查询后返回来的用户信息
			     $('#myUpdateModal').modal().on('shown.bs.modal',
			    	 ...
			    	 $("#myUpDepartment").val(data.departmentId);
			     })
		 }
	})
}
...
function updateUser(){
	...
	$.ajax({
		type:'post',//提交方式
		url:'${pageContext.request.contextPath}/user/saveUpdateUser',//修改的地址
		data:data,
		dataType:'json',
		success:function(data){
			if(data == 1){
            ...
            <div class="modal-body">
                  ...
                  <form id="myUpdateForm" class="form-horizontal">
					  ...
					  <div class="form-group">
					    ...
					    <div class="col-sm-10">
					      <input type="password" class="form-control" name="userpwd" id="inputuserPwd" placeholder="请输入密码">
					    </div>
					  </div>
					  <div class="form-group">
					  ...
					  <div class="form-group">
					    <label for="myUpdatePosition" class="col-sm-2 control-label">岗位名称:</label>
					    <div class="col-sm-10">
                      ...
                      <div class="form-group">
                          <label for="myUpDepartment" class="col-sm-2 control-label">科室名称:</label>
                          <div class="col-sm-7">
                              <select id="myUpDepartment" class="form-control" name="departmentId">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                  <form id="myAddForm" class="form-horizontal" action="">
					  ...
					  <div class="form-group">
					    <label for="postid" class="col-sm-2 control-label">岗位名称:</label>
					    <div class="col-sm-7">
					  ...
					  <div class="form-group">
					    <label for="departmentId" class="col-sm-2 control-label">科室名称:</label>
					    <div class="col-sm-7">
...
<script type="text/javascript">
	KindEditor.ready(function(K) {
		var editor = K.editor({
			uploadJson: '${pageContext.request.contextPath}/uploadJson',
			fileManagerJson: '${pageContext.request.contextPath}/fileManagerJson',
			...
			editor.loadPlugin('image', function() {
				editor.plugin.imageDialog({
					...
					clickFn : function(url, title, width, height, border, align) {
						...
						$("#myUpdateImage").attr("src","/attached/"+newUrl);
						editor.hideDialog();
					}
				});
			});
