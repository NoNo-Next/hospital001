...
<script type="text/javascript">
  $(function(){
	  $('#table').bootstrapTable({
		  url: '${pageContext.request.contextPath}/medicalrecord/queryMedicalrecord',//访问后台（json）的地址
  ...
  function myDeleteClick(id){
	  ...
	  $.ajax({
		  type:"post",
		  url:"${pageContext.request.contextPath}/medicalrecord/deleteById",
...
function myUpdateClick(medicalrecordid){
	...
	$.ajax({
		 type:"post",//请求方式
		 url:"${pageContext.request.contextPath}/medicalrecord/getById",//根据编号查询用户的信息
...
//修改事件
  function updateMedicalrecord(){
  	...
  	$.ajax({
  		type:'post',//提交方式
  		url:'${pageContext.request.contextPath}/medicalrecord/saveUpdateMedicalRecord',//修改的地址
  		data:data,
  		dataType:'json',
  		success:function(data){
  			if(data == 1){
...
function ClearInput() {
  ...
  function addMedicalrecord(){
	  ...
	  var data = $("#myAddForm").serializeArray();
	  	...
	  	$.ajax({
			type:'post',//提交方式
			url:'${pageContext.request.contextPath}/medicalrecord/addMedicalrecord',//修改的地址
			data:data,
			dataType:'json',
			success:function(data){
