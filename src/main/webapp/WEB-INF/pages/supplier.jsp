...
<script type="text/javascript">
  $(function(){
	  $('#table').bootstrapTable({
		  url: '${pageContext.request.contextPath}/supplier/getAllsupplier',//访问后台（json）的地址
 ...
 function myUpdateClick(supplierid) { 
	...
	$.ajax({
		 type:"post",//请求方式
		 url:"${pageContext.request.contextPath}/supplier/selectSupplierByid",//根据供应商id  修改  isDelete
...
function myDeleteClick(supplierid){
  	...
  	if (confirm("确定删除么？？？")){
		...
		$.ajax({
			type:"post",//请求方式
			url:"${pageContext.request.contextPath}/supplier/delSupplierByid",//根据供应商id
			data:{supplierid:supplierid},//传参数到后台
			dataType:"json",//以json格式传递数据
			success:function(data){
				if (data >0){
  ...
  function updatebtn(){
	  ...
	  var data = $("#myUpdateForm").serializeArray();
		$.ajax({
			 type:"post",//请求方式
			 url:"${pageContext.request.contextPath}/supplier/updatesupplier",//根据供应商id
			 data:data,//传参数到后台
			 dataType:"json",//以json格式传递数据
			 success:function(data){
				 location.reload(true);
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
        ...
        <form id="myUpdateForm" class="form-horizontal">
					  ...
					  <div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text"  name="chargeperson" class="form-control" id="chargeperson" placeholder="请输入负责人">
							</div>
						</div>
					  <div class="form-group">
					    <label for="address" class="col-sm-2 control-label">供应商地址:</label>
					    <div class="col-sm-10">
					      <input type="text" class="form-control" name="address" id="address" placeholder="请输入地址">
					    </div>
					  </div>
					  <div class="form-group">
					    <label for="tel" class="col-sm-2 control-label">供应商号码:</label>
					    <div class="col-sm-10">
					      <input type="text" class="form-control" name="tel" id="tel" placeholder="请输入号码">
					    </div>
					  </div>
					   <div class="form-group">
					    <label for="creditsituation" class="col-sm-2 control-label">供应商信用:</label>
					    <div class="col-sm-10">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick="updatebtn()">修改</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="myAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
        <button type="button" class="btn btn-primary" onclick="addsupp()">保存</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
	function  addsupp(){
		  var data = $("#myaddForm").serializeArray();
			$.ajax({
				 type:"post",//请求方式
				 url:"${pageContext.request.contextPath}/supplier/addsupplier",//根据供应商id
				 data:data,//传参数到后台
				 dataType:"json",//以json格式传递数据
				 success:function(data){
					 location.reload(true);
